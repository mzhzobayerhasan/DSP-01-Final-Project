...
def load_data():
    try:
        data = pd.read_csv("test.csv")  # Load dataset
...
if data is not None:
    ...
    # Get numerical features from the user
    for feature in num_features:
        user_input[feature] = st.sidebar.number_input(f"{feature}", value=float(data[feature].mean()))

    # Get categorical features from the user
    for feature in cat_features:
        user_input[feature] = st.sidebar.selectbox(f"{feature}", data[feature].unique())

    # Predict button
    if st.sidebar.button("🔮 Predict"):
        user_df = pd.DataFrame([user_input])  # Create a DataFrame from user input
        prediction = model.predict(user_df)  # Make prediction
